...
Legacy migration is always explicit and two-step. First run a read-only preview and report every path, field change, and conflict. Apply only after confirmation, then run the same preview again and require zero remaining writes. Run lifecycle migration before vocabulary consolidation. Consolidation keeps `focus_vocab_root` canonical, never deletes a base card, and blocks the whole batch when focus and base contain different non-empty bodies. After the user reviews those conflicts, pass only the confirmed exact base-card paths in `body_conflict_resolutions` with `focus` authority; keep the focus body and schedule while preserving each original base body in its archived redirect. Never infer or blanket-resolve unreviewed paths.
