...
If the current platform has no saved connection, or every saved runtime path is unavailable, ask the user where LingoTrace is located on this device. Validate that the selected directory contains `lingotrace/__init__.py` and this English skill, then append the new candidate to the current platform file. Never replace another saved candidate automatically, and never modify macOS, Windows, or Linux connection files other than the one for the current platform.
...
Before the first English learning request on each local calendar day, run the resolved runtime's `<python-command> -m lingotrace.init check-update --vault <current-vault> --runtime-root <resolved-runtime>` entry. Its platform-specific state makes later requests that day return `already_checked_today` without another fetch.
...
Legacy migration is always explicit and two-step. First run a read-only preview and report every path, field change, and conflict. Apply only after confirmation, then run the same preview again and require zero remaining writes. Run lifecycle migration before vocabulary consolidation. Consolidation keeps `focus_vocab_root` canonical, never deletes a base card, and blocks the whole batch when focus and base contain different non-empty bodies. After the user reviews those conflicts, pass only the confirmed exact base-card paths in `body_conflict_resolutions` with `focus` authority; keep the focus body and schedule while preserving each original base body in its archived redirect. Never infer or blanket-resolve unreviewed paths. New and fully migrated Vaults do not declare `base_vocab_root`; an older Vault may keep that role temporarily only while this explicit compatibility migration is still needed.
...
3. Use the public generator from the resolved LingoTrace runtime; do not reimplement its write guard, ASR routing, or note schema in an Agent-specific wrapper. Preview with `<python-command> <runtime-root>/tools/listening-transcribe-official/transcribe_listening.py <vault-relative-audio-path> --vault-root <current-vault> --lingotrace-root <runtime-root> --listenkit-root <resolved-listenkit-root> --listening-mode <extensive-or-intensive> --report-json <temporary-json-outside-vault> --dry-run`; after the guarded preview is accepted and the ordinary-language save boundary above permits writing, rerun the same command with `--apply`. Keep the report outside the Vault unless it is an intentional public artifact. English listening does not require the Japanese dictionary runtime.
...
6. If the generator returns `status: llm_merge_required`, complete the ASR merge automatically in the same user task:
   ...
   - use the current agent model's English-language judgment, whether the caller is Codex, Gemini, or another compatible agent; do not call a provider-specific API from the listening script;
...
9. Curate `0-5` productive `## 常用语块` items using model judgment. Judge them by replaceability and cross-scene communicative value, not literal length. Each item records `语块`, `类型`, `素材原句`, `替换练习`, and `交际作用`; the source sentence is only a listening anchor. Keep `daily_use_sentences` for compatibility, but store only the English chunk patterns there.
10. Save the note to the user's English learning library through `listening_notes`, then stop. Never promote freshly extracted chunks to speaking cards in the same task.
...
Before drafting vocabulary collocations or vocabulary and grammar examples, search the current target Vault for attested English sentences. Search enough candidates to fill the card's normal example slots; do not stop after the first match. Use this priority order:
...
- vocabulary with `headword`, `ipa`, `meaning_zh`, and optional word-stress, part-of-speech, English-definition, collocation, example, and confusion fields;
...
The resolved source note must embed that attachment inside `## Vocabulary`, `## 词汇`, `## 单词`, or `## 生词`. The workflow reads the source section itself and blocks image extraction when the same headword is already present as text. Do not use `image_readable: true` as a substitute for inspection evidence.
