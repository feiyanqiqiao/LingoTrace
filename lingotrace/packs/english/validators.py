...
def validate_review_materials(card: dict[str, Any]) -> CommandReport:
    ...
    if item_type == "vocab":
        ...
        errors.extend(_optional_list_errors(card, ("collocations", "confusable_with", "contrast_with")))
...
def validate_existing_review_material(card: dict[str, Any]) -> CommandReport:
    ...
    item_type = str(card.get("item_type", ""))
    if item_type == "vocab":
        if not any(field in card for field in ("headword", "ipa", "word_stress", "meaning_zh", "english_definition")):
            errors.append(
                Finding(
                    code="missing_english_field",
                    message="Vocabulary review material requires a headword, IPA, meaning, word stress, or English definition field.",
                )
            )
    elif item_type == "grammar":
