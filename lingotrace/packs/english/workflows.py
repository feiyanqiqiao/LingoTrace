...
from lingotrace.packs.english.validators import (
...
def _render_vocab_body(
    ...
    core: list[str] = []
    accent = fields.get("word_stress")
    if accent:
        stress_label = item.get("stress_label")
        core.append(f"- 重音：{stress_label} ({accent})" if stress_label else f"- 重音：{accent}")
    if fields.get("part_of_speech"):
        core.append(f"- 词性：{fields['part_of_speech']}")
    if core:
        sections.append("## 核心\n\n" + "\n".join(core))

    if fields.get("english_definition"):
    ...
    if item.get("example_en"):
        examples.insert(0, (str(item["example_en"]), str(item.get("example_zh") or "")))
    if examples:
        lines: list[str] = []
        for english, chinese in examples:
...
def _render_grammar_body(
    ...
    if item.get("typical_example_en"):
        quick.append(f"- 典型例句：{item['typical_example_en']}")
    if quick:
        sections.append("## 快速复习\n\n" + "\n".join(quick))
    ...
    if fields.get("cefr"):
        register_lines.append(f"- CEFR：{fields['cefr']}")
    if register_lines:
        ...
        for index, usage in enumerate(usage_sections, start=1):
            ...
            if examples:
                ...
                for english, chinese in examples:
                    ...
                    detail_lines.append(f"  - {english}{suffix}")
            if not detail_lines:
                continue
...
def _example_entries(value: Any) -> list[tuple[str, str]]:
    ...
    for entry in values:
        if isinstance(entry, dict):
            english = str(entry.get("en") or entry.get("english") or "").strip()
            chinese = str(entry.get("zh") or entry.get("chinese") or "").strip()
        else:
            english = str(entry).strip()
            chinese = ""
        if english:
            examples.append((english, chinese))
    return examples
...
def _markdown_vocabulary_section(text: str) -> str | None:
    match = re.search(r"(?m)^##\s+(?:Vocabulary|词汇|单词|生词)\s*$", text)
    if match is None:
        return None
