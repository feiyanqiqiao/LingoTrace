...
agent_skill_re='^\.agents/skills/speckit-[^/]+/SKILL\.md$'
...
while IFS= read -r file; do
  ...
  if [[ "$file" =~ $specify_private_re ]]; then
    bad_files="${bad_files}${file} :: local Spec Kit state is never allowed"$'\n'
    continue
  fi

  if [[ "$file" == .agents/* ]] && ! [[ "$file" =~ $agent_skill_re ]]; then
    bad_files="${bad_files}${file} :: agent-local state is never allowed"$'\n'
    continue
  fi
