...
本项目当前由 LingoTrace core 和 Japanese language pack 承接五个日语工作流，覆盖外语学习“输入 -> 内化 -> 复习 -> 输出”的全生命周期：

1. **听力笔记**：`lingotrace.packs.japanese.workflows:listening_notes`
2. **来源笔记**：`lingotrace.packs.japanese.workflows:source_notes`
3. **复习材料维护**：`lingotrace.packs.japanese.workflows:review_materials`
4. **生活口语卡**：`lingotrace.packs.japanese.workflows:speaking_cards`
5. **每日复习结算**：`lingotrace.packs.japanese.workflows:review_rollover`

这些入口必须通过 Vault context、Japanese pack manifest 和 core write guard 运行。旧的 Vault 内嵌入口不再是公共仓库的目标运行模式。
