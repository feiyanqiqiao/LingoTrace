...
LingoTrace 不是课程、教材内容仓库或通用背单词 App。它提供的是一套由 AI 驱动、基于本地 Markdown 知识库的外语学习系统骨架：
...
- **🧠 AI Agent（自动化执行与记忆大脑）**：负责听力转写、长文语法解析、自动提炼词汇/口语卡、计算艾宾浩斯复习周期与每日结算。
- **📦 Obsidian Vault（本地数据资产库）**：所有学习笔记、复习卡片均以纯文本 Markdown 和标准 Frontmatter 保存在本地，无私有云绑定。
- **🖥️ Obsidian 桌面端（官方 GUI 看板与点读终端）**：官方推荐的图形交互界面，负责 Markdown 沉浸排版、双链跳转、精听音频切片点读，以及 `.base` 数据库全景看板的渲染与自测。
...
日常使用时，直接对 AI agent 说学习目标，无需记忆内部路径或函数名。

## 双端协同动线

| 步骤 | 操作界面 | 典型指令 / 操作 | 作用 |
| --- | --- | --- | --- |
| 1. 生成材料 | AI Agent 窗口 | “请把这段音频做成精听稿” / “把这个词加入复习” | Agent 提取要点并在 Vault 中生成 Markdown 卡片 |
| 2. 沉浸学习 | Obsidian 桌面端 | 点击音频切片跟读、阅读语法解析 | 利用 Obsidian 桌面端获得流畅的排版与点读体验 |
| 3. 看板自测 | Obsidian 桌面端 | 打开 `Total Training Dashboard` 看板自测今日卡片 | 基于 Obsidian Base 表格直观查看今日到期卡片 |
| 4. 每日结算 | AI Agent 窗口 | “今天复习结束了，帮我结算” | Agent 自动推进复习阶段并更新下次复习日期 |
...
精听稿应包含真实音频切片，不能只生成空的 Markdown 结构。系统会先检查音频和切片工具；如果需要修改已有笔记，会先停下来确认，不会覆盖你已经手工整理过的笔记。生成后，在 Obsidian 桌面端中打开笔记，即可点击切片直接点读与跟读。
...
Japanese 与 English 语言包提供可复用模板和默认视图，例如总训练面板模板（`.base`）。你可以把这些模板复制到自己的学习库后再按个人习惯调整。
...
1. 准备一个自己的 Obsidian 日语或英语学习库（Vault），并在 Obsidian 桌面端中打开。
2. 安装或打开能读取 LingoTrace Agent Skill 的 AI agent，并将该 Vault 设置为工作区。
3. 选择一个小材料，例如一段短音频或一页自学笔记。
4. 用自然语言要求 agent 生成听力笔记或来源笔记。
5. 从笔记里人工确认几个词汇、语法点或口语句。
6. 让 agent 建少量复习卡。
7. 在 Obsidian 桌面端的 `Total Training Dashboard` 中完成当天卡片自测。
8. 切回 Agent 窗口，让 agent 做一次每日复习结算。
