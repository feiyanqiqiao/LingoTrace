...
开发者的实际学习 Vault 同样在每天第一次学习前检查正式上游。如果 Vault 连接的是个人 fork，检查命令可以 fetch 并用中文概括上游变化，但即使用户说“更新”，也不得在学习任务里自动 pull、merge 或 rebase。告诉用户这是自己改造过的开发仓库，直接操作可能产生冲突，请其切换到完整开发工作区，按本协议第 4 节自行同步。

用户可以忽略提示并继续学习。日检提醒不能代替开发任务开始前的完整 `fetch --all --prune`、分支比较和测试流程。
